...
class DIFFHOI_S_Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_encoder_layers=6,
        ...
        self.laynorm = nn.LayerNorm(d_model, d_model)
    ...
    def forward(self, src, mask, query_embed_h, query_embed_o, pos_guided_embed, pos_embed,sd_decoder):
        # flatten NxCxHxW to HWxNxC
        bs, c, h, w = src.shape
        bs, c, h_0, w_0 = sd_decoder.shape
        src = src.flatten(2).permute(2, 0, 1)
        sd_decoder = sd_decoder.flatten(2).permute(2, 0, 1)
...
def build_diffhoi_s_transformer(args):
    return DIFFHOI_S_Transformer(
